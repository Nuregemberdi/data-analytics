...
-- 14 мүчө. Эң көбү Tim Rownam — 408 (орточосу 109).
-- Эки derived table: mb (ар бир мүчөнүн саны) жана counts (орточону табуу үчүн).


-- ⚠️ БИРИНЧИ ВАРИАНТ — CROSS JOIN менен. ЧЫГАРА АЛБАЙ ТЕОРИЯ КАРАДЫМ.
--    ... CROSS JOIN (SELECT AVG(counts.cnt) AS avg_bookings FROM (...) counts) avg_table
--    WHERE mb.booking_count > avg_table.avg_bookings
--
-- Query ТУУРА иштеген. Бирок мугалим: «CROSS JOIN'ду өткөн жокпуз, кайдан келди?»
--
-- ЭРЕЖЕ: ТҮШҮНДҮРӨ АЛБАГАН САПТЫ КОДУҢДА КАЛТЫРБА.
--   Теория кароо — ката эмес, күнүмдүк иш.
--   Тыюу салынганы — даяр жоопту көчүрүү.
--   Маекте так ошол сап жөнүндө сурашат.
-- Ошондуктан CROSS JOIN алынып, WHERE'деги подзапроско алмаштырылды.
--
-- CROSS JOIN ДЕГЕН ЭМНЕ: ал — Cartesian product, башка ат менен.
--   ON шарты ЖОК: сол жактын ар бир катары оң жактын ар бир катары менен жупташат.
--   Коопсуз болгон жалгыз учур: оң жакта БИР ГАНА катар турса (бир сан).
--   Ошондо ал сан ар бир катарга жабыштырылат, катар саны өзгөрбөйт.


-- ============================================
-- 4-ЖУМА ЖАБЫЛДЫ · 30/30
-- ============================================
-- Жалпы маселе: 30 (2-жума) + 40 (3-жума) + 30 (4-жума) = 100
-- ФАЗА 1'ДИН 1-КРИТЕРИЙИ (100 маселе) АТКАРЫЛДЫ.
--
-- Калган эки критерий:
--   - терезе функциясын жардамсыз жазуу (5-жума)
--   - БАШКАНЫН туура эмес query'син таап оңдоо (сынак коюла элек)
--
-- 5-ЖУМА: CTE (WITH) жана терезе функциялары
--   ROW_NUMBER · RANK · LAG · SUM() OVER. Максат 25 маселе.
--   CTE'нин кереги 30-тапшырмада өз көзү менен көрүлдү:
--   бир эле эсептөө query'де ЭКИ ЖОЛУ жазылды.
--
-- Ачык калган аналитикалык суроо:
--   Massage Room 2 эмне үчүн дээрлик колдонулбайт? (rate 2.25)
--   Баа божомолу жокко чыкты — баалар бирдей (35 / 80).
